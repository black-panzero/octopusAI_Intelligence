...
const RegisterForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await authApi.register({ email: form.email.trim(), password: form.password, full_name: form.full_name.trim() || null });
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <div className="relative">
          ...
          <input name="full_name" type="text" value={form.full_name} onChange={handleChange} autoComplete="name"
                 placeholder="John Doe" className={ic} />
        </div>
      </div>
      <div>
